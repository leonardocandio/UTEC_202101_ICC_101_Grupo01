...
dir_lvls = os.path.split(current_dir)[0]
...
TILE_TYPES = len(os.listdir(current_dir + "\\img\\tile"))
level = 0
current_tile = 0
scroll_left = False
scroll_right = False
...
# load images
pine1_img = pygame.image.load(current_dir + "/img/Background/pine1.png").convert_alpha()
pine2_img = pygame.image.load(current_dir + "/img/Background/pine2.png").convert_alpha()
mountain_img = pygame.image.load(
    current_dir + "/img/Background/mountain.png"
).convert_alpha()
sky_img = pygame.image.load(
    current_dir + "/img/Background/sky_cloud.png"
).convert_alpha()
# store tiles in a list
img_list = []
for x in range(TILE_TYPES):
    img = pygame.image.load(f"{current_dir}/img/tile/tile{x}.png").convert_alpha()
    img = pygame.transform.scale(img, (TILE_SIZE, TILE_SIZE))
    img_list.append(img)

save_img = pygame.image.load(current_dir + "/img/save_btn.png").convert_alpha()
load_img = pygame.image.load(current_dir + "/img/load_btn.png").convert_alpha()
...
while run:
    ...
    if save_button.draw(screen):
        # save level data
        with open(
            f"{dir_lvls}\\assets\\maps\\levels\\level{level}_data.csv",
            "w",
            newline="",
        ) as csvfile:
